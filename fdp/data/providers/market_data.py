...
from fdp.core.service_tier import tiered_service_manager
...
class MultiSourceMarketDataManager:
    def __init__(self, rate_limiter: TokenBucketRateLimiter, circuit_breaker: CircuitBreaker):
        ...
        self.tier_manager = tiered_service_manager
    ...
    async def fetch_ohlcv(self, symbol: str, days: int = 365, interval: str = "1d",
        ...
        for provider in provider_order:
            service_config = self.tier_manager.get_service_config(provider)
            if await self.tier_manager.check_and_record_usage(provider):
                available_providers.append(provider)
            else:
                logger.warning("provider_not_available", provider=provider, tier=service_config.tier.value)
            ...
            async with self.semaphore:
                try:
                    ...
                    if not df.empty:
                        ...
                        await self.rate_limiter.record_spend(provider, 0.001)
        ...
        tasks = [try_provider(p) for p in available_providers]
    ...
    async def _fetch_yahoo(self, symbol: str, days: int, interval: str) -> pd.DataFrame:
        await self.rate_limiter.acquire("yahoo", symbol, config.rl_yahoo)
        def _sync_fetch():
            ticker = yf.Ticker(symbol)
            ticker.session.headers.update({
                "User-Agent": "FinDashPro/3.1.3 (mailto:contact@example.com)"
            })
